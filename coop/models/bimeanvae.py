...
class BiMeanVAE(Model):
    def __init__(
        ...
        self.proj_dec = nn.Sequential(nn.Linear(latent_dim, 2 * hidden_size), nn.Tanh())
    ...
    def forward(
        ...
        **kwargs
        ...
        packed_embed = nn.utils.rnn.pack_padded_sequence(
            embed, input_length, batch_first=True, enforce_sorted=False
        )
    ...
    def _recon_loss(self, tgt_tensor: torch.Tensor, z: torch.Tensor):
        ...
        for t in range(max_len - 1):
            ...
            non_masked_embeddings = hx.masked_select(tgt_mask.unsqueeze(-1)).view(
                -1, self.hidden_size
            )
            non_masked_outs = self.output_layer(non_masked_embeddings)
            recon_losses.append(
                F.cross_entropy(non_masked_outs, non_masked_targets, reduction="sum")
            )
    ...
    def generate(
        ...
        start_predictions = torch.full(
            (bz,), fill_value=self.bos_id, dtype=torch.long, device=device
        )
        hx, cx = torch.chunk(self.proj_dec(z), 2, dim=-1)
        decoder_state = {"z": z, "hx": hx, "cx": cx}
        self.beam.beam_size = num_beams
        self.beam.max_steps = max_tokens
        all_top_k_predictions, _ = self.beam.search(
            start_predictions, decoder_state, self.step
        )
        self.bad_words.clear()
        return all_top_k_predictions[:, 0]

    @torch.no_grad()
    def step(self, last_predictions: torch.Tensor, state: Dict[str, torch.Tensor]):
        z = state["z"]
        hx, cx = self.decoder(
            torch.cat((self.embed(last_predictions), z), dim=-1),
            (state["hx"], state["cx"]),
        )
    ...
    @staticmethod
    def klw(step: int, interval: int, r: float = 0.8, t: float = 0.0, s: int = 10000):
        if step < s:
            return 0.0
        else:
