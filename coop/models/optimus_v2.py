...
class Optimus(Model):
    def __init__(
        ...
        self.decoder = OptimusDecoder(decoder_config, latent_dim=latent_dim, pad_id=pad_id)
    ...
    def forward(
        ...
        cls_vec = self.encoder(**src, **encoder_kwargs, return_dict=True).pooler_output
...
class OptimusGPT2(GPT2PreTrainedModel):
    def __init__(self, config, latent_dim: int):
        ...
        self.wpe = nn.Embedding(config.n_positions + 1, config.n_embd)
