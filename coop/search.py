...
def brute_force_gen(
    ...
    for i, x in enumerate(data):
        ...
        outs.append(
            [
                {
                    "selected": [x["reviews"][i] for i in idx],
                    "reviews": x["reviews"],
                    "summary": x["summary"],
                    "predicted": gen,
                    "idx": idx,
                }
                for idx, gen in zip(idxes, gens)
            ]
        )
    return outs
...
def main(log_dir_or_file, split):
    ...
    if config["model"]["type"].lower() == "bimeanvae" and config["data_dir"].endswith(
        "amzn"
    ):
    ...
    model.load_state_dict(
        torch.load(
            log_dir / "pytorch_model.bin", map_location=lambda storage, loc: storage
        )
    )
    if torch.cuda.is_available():
        model.cuda()

    dev_gen = brute_force_gen(
        model, dev, tgt_tokenizer, bad_words_ids=bad_words_ids, split=split
    )
    test_gen = brute_force_gen(
        model, test, tgt_tokenizer, bad_words_ids=bad_words_ids, split=split
    )
    ...
    with Pool(cpu_count()) as p:
        for func in (avg, overlap, oracle):
            ...
            for key, val in (("dev", dev_gen), ("test", test_gen)):
                ...
                rouge_score = {
                    "_".join((metric, k)): v
                    for metric, vs in rouge_score.items()
                    for k, v in vs.items()
                }
                coop[name][key] = {
