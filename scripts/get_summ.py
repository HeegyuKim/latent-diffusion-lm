...
def amzn(output_dir):
    ...
    os.system(
        "git clone https://github.com/ixlan/Copycat-abstractive-opinion-summarizer.git"
    )

    for sp in ("dev", "test"):
        ins = []
        for x in csv.DictReader(
            open(f"Copycat-abstractive-opinion-summarizer/gold_summs/{sp}.csv"),
            dialect="excel-tab",
        ):
            ins.append(
                {
...
def yelp(output_dir):
    ...
    for x in csv.DictReader(io.StringIO(file)):
        ins.append(
            {
                "reviews": [
                    strip_text(x[f"Input.original_review_{i}"]) for i in range(8)
                ],
